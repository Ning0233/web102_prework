...
const toalContributions = GAMES_JSON.reduce((toal, game) => {
    return toal + game.backers;
}, 0);
contributionsCard.innerHTML = toalContributions.toLocaleString();
...
const unfundedGamesCount = GAMES_JSON.reduce((count,game) => {
    return !game.funded ? count + 1: count;
}, 0)

// create a string that explains the number of unfunded games using the ternary operator
const message = `
    A total of $${totalRaised.toLocaleString()} has been raised for ${GAMES_JSON.length} game${GAMES_JSON.length === 1 ? '' : 's'}.
    Currently, there ${unfundedGamesCount === 1 ? 'is' : 'are'} ${unfundedGamesCount} game${unfundedGamesCount === 1 ? '' : 's'} still unfunded.
`;
...
const decriptionContainer = document.getElementById("description-container")
